    ...
    private class Patches
    {
      ...
      private static string makeHoverText(CustomSecretNoteData noteData)
      {
        ...
        if (!string.IsNullOrEmpty(content))
        {
          ...
          string[] split = content.Split(System.Environment.NewLine);
          int max = 10;
          if (split.Length > max)
          {
            content = string.Join(System.Environment.NewLine,
                    split.Take(max)) + System.Environment.NewLine + "(...)";
          }
          return title + System.Environment.NewLine + System.Environment.NewLine + content;
        }
        return title;
      }
    }
